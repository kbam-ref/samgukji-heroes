...
const UNIT_H = 1.45;             // 유닛 높이(월드) — 축소(수석: 너무 큼)
const ENEMY_H = 1.2;             // 적 기준 높이(× 사이즈 배수)
  ...
  const upd = (n, bob) => {
    ...
    if (n.isModel) {
      ...
      if (n.mixer) { // 스켈레탈 — 팔다리 애니(적=걷기, 영웅=공격/대기)
        ...
        n.modelObj.rotation.z = n.isAttacker ? Math.sin(clock * 1.6 + n.phase) * 0.02 : 0; // 영웅 대기 미세 흔들림
      } else {
