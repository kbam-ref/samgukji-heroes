...
export const DEFENSE = {
  ...
  wave: {
    ...
    spawnInterval: 0.8, // 2026-07-22 수석: 80초 동안 100마리 출현(100×0.8=80초 스폰) → 라운드타임 100초 중 20초 여유
    loseAt: 100, // 2026-07-21 수석 재지시: 살아있는 적 100 누적 = 게임오버(원상복구)
    // 2026-07-22 수석: 라운드타임 80→100. 80초간 100마리 다 나오고도 20초 여유가 남게(다 잡을 시간).
    roundTime: 100,
    roundClearGold: 100, // 2026-07-22 수석: 라운드 돌파 보상 100 (기대치에 맞춤)
    // 2026-07-21: 170→140→100 (수석 "난이도 과하다"). 초반 확실히 완화, 후반 램프도 소폭 낮춤.
    hpBase: 100,
    ...
    // 2026-07-23 상품화 리메이크: 16라운드부터 후반 지수 분리. 합성 사다리·특성·경제 강화로 플레이어 파워가
    //   크게 올라(초월 도달 9/14) 1.21로 최종 국면을 조여 승리를 '결전'으로 — 시뮬(준최적 정책) 승률 6/14,
    //   초월런 6/9(초월을 완성해도 적벽에서 1/3은 진다). 실플레이 체감 승률은 이보다 낮다.
    hpLateStage: 16,
    hpPerStageLate: 1.21,
    speed: 8,
    // 재화 — 개체당 골드에 스테이지 스케일(검증 C1: flat 수입 vs 기하 HP → 경제 붕괴).
    // 개체 골드 = goldPerKill × size.gold × (보스면 boss.goldMult) × goldPerStage^(stage-1)
    goldPerKill: 0.5, // 2026-07-22 수석: 10킬당 5골드(=0.5/킬 균일). 체급 배수 제거(아래 sizes.gold 전부 1).
    // 2026-07-23 감사: 수입 평탄 vs 난이도 기하급수 → 후반 경제 디플레(소환도 단련도 못 함). 라운드당 +5% 복리로
    //   후반에도 '벌어서 강해지는' 루프가 돌게. (R10 ≈ 0.78/킬, R20 ≈ 1.26, R30 ≈ 2.06)
    goldPerStage: 1.05,
    ...
    boss: {
      everyStages: 5,
      count: 2,
      // 2026-07-23 감사: 보상 균일 50G는 '스펀지 잡는 노동'. 라운드 비례로 — R5 적장 85G, R30 채모 210G(마리당).
      killGold: 60,
      killGoldPerStage: 5,
      hpMult: 9, // 2026-07-23 감사: 12는 긴장이 아니라 지루한 스펀지 — 스파이크는 유지하되 처치 가능한 선(9)으로.
      speedMult: 0.5,
      goldMult: 8,
      ...
      // 2026-07-22 수석: 보스 처치 무료 유닛 지급 폐지(전설+ 쉽게 얻어 판 쉬워짐). 보상은 killGold(50)뿐.
      grant: [],
    },
  },
  ...
  summon: {
    ...
    startGold: 60, // 2026-07-23 감사: 첫 90초 소환 가뭄(무료 5뽑 후 ~2분 무행동) 해소 — 초반 1~2회 추가 소환분
    ...
    // 2026-07-23 감사: 구 천장(50뽑에 그 1회 등급+1)은 2,000G 대가로 '일반→희귀'가 될 수도 있는 장식이었다.
    //   → 40뽑마다 '전설(4성) 이상 확정'. 눈에 보이는 진짜 안전망(헌장 #3: 천장까지 남은 횟수 상시 표시).
    pity: { pulls: 40, effect: 'minRarity', min: 4 },
  },
  ...
  gamble: {
    cost: 250,
    perPip: 26,      // 2026-07-23 감사: RTP 103%→94% — '의무 클릭'이 아니라 재미로 굴리는 도박으로(잭팟 스릴 유지)
    doubleGold: 500, // 같은 수(1·1, 2·2 …) 잭팟
    cooldown: 60,    // 2026-07-21 수석: 1분에 1회
    // 2026-07-23 감사: 승급 도박이 소환·합성·천장을 전부 우회하는 지배 전략(200G/20%) → 300G/15%로 보정.
    up: { cost: 300, chance: 0.15 },
  },

  // ── 합성 — 2026-07-20 수석 지시: '같은 영웅' 3장 → 상위 등급 랜덤 1장(수동). 전설은 합성 없음 ──
  // (소환가 50·작은 등급별 풀이라 같은 장수 3장 모으기가 실제로 가능 → C5 판단 뒤집음)
  merge: {
    need: 5, // 기본값(폴백). 실제로는 아래 needByRarity가 우선.
    // 2026-07-23 감사: 전 등급 5장 요구는 등급 사다리를 4성에서 끊는다(같은 전설 5장 = 사실상 불가 →
    //   초월 도달 경로가 0.2% 생소환뿐). 높은 등급일수록 필요 장수를 줄여 사다리를 끝까지 잇는다.
    //   index = 재료 등급(1~5). 1·2성 5장 → 3성 4장 → 4성 3장 → 5성(신화) 2장 = 초월.
    needByRarity: [0, 5, 5, 4, 3, 2],
    // 승급체는 재료 중 '최고 단련 레벨'을 계승, 나머지의 단련 골드는 50% 환급(검증 C6).
  ...
  unit: {
    ...
    // 2026-07-22 수석: "전설·신화 한둘로 판이 쓸려 너무 쉬워짐 → 초월쯤 돼야 지금 밸런스". 전설/신화 동시타격 대폭 너프.
    //   판 쓸이의 핵심은 multi(동시타격)이므로 4=1인, 5=2인으로 줄이고 사거리/데미지도 완화. 초월(6)만 강력 유지.
    byRarity: {
      ...
      4: { range: 34, cooldown: 0.82, dmg: 1.4, multi: 1 },  // 전설 — 단일 강타(3인동타 폐지)
      5: { range: 44, cooldown: 0.74, dmg: 1.62, multi: 2 }, // 신화 — 2인 동타
      6: { range: 62, cooldown: 0.62, dmg: 2.1, multi: 5, aoe: { interval: 10, dmgMult: 4 } }, // 초월 — 여기서부터 판이 풀린다
    },
