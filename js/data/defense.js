...
export const DEFENSE = {
  ...
  wave: {
    ...
    // 2026-07-20: 95→130 (수석 "여전히 쉬움"). 시뮬: 엔드게임 로스터(6성4+고단련+광역기)도 동시 적 89/100 아슬아슬.
    hpBase: 130,
    // 1.175→1.2. r30 ≈ 130×1.2^29 ≈ 30,000×(크기·보스). 잘 키운 6성 로스터라야 30 도달(약하면 20~25 벽).
    hpPerStage: 1.2,
    hpPerIndex: 0.01,
    speed: 8,
    // 재화 — 개체당 골드에 스테이지 스케일(검증 C1: flat 수입 vs 기하 HP → 경제 붕괴).
    // 개체 골드 = goldPerKill × size.gold × (보스면 boss.goldMult) × goldPerStage^(stage-1)
    goldPerKill: 1, // 기본 1골드/처치
  ...
  unit: {
    ...
    // 2026-07-20 수석: 50골드부터 레벨당 +1골드(선형). 공격/공속 각각.
    elemUpgrade: {
      costBase: 50,
      costStep: 1,
      maxLevel: 30,
